...
export default function SceneViewerPage() {
  ...
  // Sun angle from hour
  const sunX = Math.cos((hour / 24) * Math.PI * 2) * 40;
  const sunY = -Math.abs(Math.sin((hour / 24) * Math.PI * 2)) * 30 - 10;
        ...
        <div className="gc-mv-body">
          ...
          <div className="gc-mv-rail">
            {RAIL.map(({ id, icon, label }) => (
              <button key={id} className={`gc-mv-rail-btn ${rail === id ? "active" : ""}`}
                    ...
                    <div className="gc-basemap-grid" style={{ gridTemplateColumns:"repeat(2,1fr)" }}>
                      {BASEMAPS_3D.map(bm => (
                        <div key={bm.id} className={`gc-basemap-item ${basemap.id === bm.id ? "active" : ""}`} onClick={() => setBasemap(bm)}>
                          <div className="gc-basemap-thumb" style={{ background: bm.id === "dark" ? "#1a1a2e" : bm.id === "imagery" ? "#0d1b2a" : "#e8eff7" }}>{bm.emoji}</div>
                  ...
                  {rail === "daylight" && (
                    <div style={{ padding:14 }}>
                      <div style={{ position:"relative", height:80, background:"linear-gradient(180deg,#87ceeb,#ffd700)", borderRadius:8, marginBottom:14, overflow:"hidden" }}>
                        <div style={{ position:"absolute", left:`calc(50% + ${sunX}px)`, top:`calc(50% + ${sunY}px)`, fontSize:20 }}>☀️</div>
                      </div>
                      ...
                      <div style={{ display:"flex", gap:4, marginTop:8 }}>
                        {["Jan","Apr","Jul","Oct"].map(m => (
                          <button key={m} className="gc-btn gc-btn-default gc-btn-xs">{m}</button>
                        ))}
                      </div>
                    </div>
                  )}
          ...
          {/* 3D Scene canvas */}
          <div className="gc-scene-canvas" style={{ transform:`perspective(800px) rotateX(${tilt * 0.3}deg)`, transformOrigin:"bottom center" }}>
            ...
            <div className="gc-mv-status">
              <span>5.50°N, 6.25°E</span>
              <span>·</span>
              <span>Altitude 4,200 m</span>
              <span>·</span>
              <span>Tilt {tilt}°</span>
              <span>·</span>
              <span>WGS 1984</span>
