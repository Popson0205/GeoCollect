...
import {
  ...
  Maximize2, Minimize2, CheckCircle2, X, SlidersHorizontal, Type, Filter,
  Sparkles, BarChart2, ZoomIn, ZoomOut,
} from "lucide-react";

// ── Types ─────────────────────────────────────────────────
type RailTool  = "layers" | "basemap" | "legend" | "bookmarks" | "search" | "print" | "measure" | "sketch";
...
const RAIL: { id: RailTool; icon: React.ReactNode; label: string }[] = [
  ...
  { id:"bookmarks", icon:<BookOpen size={16}/>,        label:"Bookmarks" },
    ...
    <div className="gc-page" style={{ height:"100vh", overflow:"hidden" }}>
      ...
      <div className="gc-mv">
        ...
        <div className="gc-mv-toolbar">
          ...
          <button className="gc-btn gc-btn-ghost gc-btn-sm" style={{ gap:5 }}>
            <ChevronDown size={12}/> Save As
          </button>
          <div className="gc-mv-toolbar-divider"/>
          ...
          <div className="gc-mv-toolbar-divider"/>
          <button className="gc-btn gc-btn-ghost gc-btn-icon" onClick={() => setFull(!full)} title={full ? "Exit fullscreen" : "Fullscreen"}>
          ...
          <div className="gc-mv-rail">
            {RAIL.map(({ id, icon, label }) => (
              <button key={id} className={`gc-mv-rail-btn ${railTool === id ? "active" : ""}`}
                onClick={() => toggleRail(id)} title={label}>
                {icon}
              </button>
            ))}
            <div className="gc-mv-rail-divider"/>
            <button className="gc-mv-rail-btn" title="Analysis Tools" onClick={() => toggleRail("search")}>
              <Sparkles size={16}/>
            </button>
          </div>
                ...
                <div className="gc-mv-panel-body">
                  ...
                  {railTool === "layers" && (
                    <div>
                      ...
                      {layers.map(l => (
                        <div key={l.id} className="gc-layer-row" style={{ background: selLayer === l.id ? "var(--c-blue-light)" : undefined }}
                          ...
                          <div className="gc-layer-actions">
                            <button className="gc-btn gc-btn-ghost gc-btn-icon" onClick={e => { e.stopPropagation(); toggleLayer(l.id); }} title={l.visible ? "Hide" : "Show"}>
                              {l.visible ? <Eye size={12}/> : <EyeOff size={12}/>}
                            </button>
                            <button className="gc-btn gc-btn-ghost gc-btn-icon" onClick={e => { e.stopPropagation(); removeLayer(l.id); }} title="Remove">
                              <Trash2 size={11}/>
                            </button>
                          </div>
                        </div>
                  ...
                  {railTool === "basemap" && (
                    <div className="gc-basemap-grid">
                      {BASEMAPS.map(bm => (
                        <div key={bm.id} className={`gc-basemap-item ${basemap.id === bm.id ? "active" : ""}`}
                          onClick={() => setBasemap(bm)}>
                          <div className="gc-basemap-thumb" style={{ background: bm.id === "dark" ? "#1a1a2e" : bm.id === "satellite" ? "#0d1b2a" : bm.id === "ocean" ? "#1a3a5c" : "#e8eff7" }}>
                            {bm.emoji}
                          </div>
                ...
                <div className="gc-mv-props-body">
                  {propTab === "styles" && (
                    <>
                      <div className="gc-prop-group">
                        ...
                        <select className="gc-prop-input">
                  ...
                  {propTab === "filters" && (
                    <div className="gc-prop-group">
                      ...
                      <div className="gc-prop-row">
                        <label>Field</label>
                        <select className="gc-prop-input"><option>status</option><option>name</option><option>area_m2</option></select>
                      </div>
                      <div className="gc-prop-row">
                        <label>Operator</label>
                        <select className="gc-prop-input"><option>is</option><option>is not</option><option>contains</option></select>
                      </div>
                      <div className="gc-prop-row">
                        <label>Value</label>
                        <input className="gc-prop-input" placeholder="active"/>
                      </div>
