...
function MapContent() {
  ...
  return (
    <div style={{ display: 'flex', height: 'calc(100vh - 52px)', position: 'relative' }}>
      {/* Layer sidebar */}
      <div style={{ width: sidebarOpen ? '280px' : '0', flexShrink: 0, background: 'white', borderRight: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', overflow: 'hidden', transition: 'width 0.2s ease' }}>
        <div style={{ padding: '16px', borderBottom: '1px solid #f1f5f9', flexShrink: 0 }}>
          <div style={{ fontSize: '11px', fontWeight: 700, color: '#94a3b8', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '12px' }}>Feature Layers</div>
          {loadingProjects ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              {projects.map(p => (
                <button key={p.id} onClick={() => setSelectedId(p.id)} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderRadius: '10px', border: 'none', cursor: 'pointer', textAlign: 'left', width: '100%', background: selectedId === p.id ? '#eff6ff' : 'transparent', transition: 'background 0.15s' }}
      ...
      {/* Toggle sidebar */}
      <button onClick={() => setSidebarOpen(v => !v)} style={{ position: 'absolute', left: sidebarOpen ? '268px' : '0', top: '50%', transform: 'translateY(-50%)', zIndex: 10, background: 'white', border: '1px solid #e2e8f0', borderRadius: '0 8px 8px 0', width: '20px', height: '48px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '2px 0 8px rgba(0,0,0,0.06)', transition: 'left 0.2s ease', color: '#64748b' }}>
...
export default function MapViewerPage() {
  return (
    <AppShell activeApp="map" title="Map Viewer">
      <Suspense fallback={
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 'calc(100vh - 52px)', flexDirection: 'column', gap: '14px' }}>
