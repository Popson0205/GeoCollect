...
import { Users, Settings, Shield, CreditCard, BarChart2, Plus, Search, MoreHorizontal, Mail } from "lucide-react";
...
const NAV: { id: OrgTab; label: string; icon: React.ReactNode }[] = [
  ...
  { id:"status",    label:"Status",             icon:<BarChart2 size={14}/> },
        ...
        <div className="gc-org-content">
            ...
            <div>
              ...
              <div className="gc-card" style={{ overflow:"hidden" }}>
                <table className="gc-table" style={{ borderRadius:0, border:"none", boxShadow:"none" }}>
                  <thead><tr>
                    <th>Member</th><th>Username</th><th>Role</th><th>Last Login</th><th>Status</th><th></th>
                  </tr></thead>
                  <tbody>
                    {filtered.map(m => (
                      <tr key={m.id}>
                        ...
                        <td>
                          <span className={`gc-badge ${m.status === "active" ? "gc-badge-public" : "gc-badge-private"}`} style={{ display:"inline-flex" }}>
                            {m.status}
                          </span>
                        </td>
                        <td>
                          <button className="gc-btn gc-btn-ghost gc-btn-icon"><MoreHorizontal size={14}/></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {tab === "licenses" && (
            <div>
              ...
              <div className="gc-card" style={{ padding:18 }}>
                ...
                {[["GeoCollect Studio","4 / 10 used","#0079c1"],["Field Collector App","2 / 20 used","#2e7d32"],["Map Viewer","4 / 10 used","#e65100"]].map(([name,usage,color]) => (
                  <div key={name} style={{ marginBottom:16 }}>
                    ...
                    <div className="gc-progress-bar">
                      <div className="gc-progress-fill" style={{ width: usage.includes("4") ? "40%" : "10%", background: color }}/>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {tab === "status" && (
            <div>
              ...
              <div className="gc-card" style={{ overflow:"hidden" }}>
                {[["API Service (port 3001)","Operational"],["Geo API (port 3002)","Operational"],["GeoServer (port 8080)","Operational"],["PostgreSQL + PostGIS","Operational"],["Redis","Operational"],["MinIO Object Storage","Operational"]].map(([svc,status]) => (
          ...
          {tab === "settings" && (
            <div style={{ display:"flex", flexDirection:"column", gap:16, maxWidth:620 }}>
              ...
              <div className="gc-card" style={{ padding:18 }}>
                ...
                {[["Require multi-factor authentication","MFA"],["Allow access via trusted domains only","Trusted Domains"],["Enable HTTPS only","HTTPS"]].map(([lbl,key]) => (
                  <label key={key} style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 0", borderBottom:"1px solid var(--c-border-light)", cursor:"pointer", fontSize:13 }}>
                    <input type="checkbox" defaultChecked={key === "HTTPS"} style={{ accentColor:"var(--c-blue)" }}/>
                    {lbl}
                  </label>
                ))}
              </div>
