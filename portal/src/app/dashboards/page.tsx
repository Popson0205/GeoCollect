...
const ICONS: Record<WType,React.ReactNode> = { bar:<BarChart2 size={28}/>, line:<LineChart size={28}/>, pie:<PieChart size={28}/>, map:<Map size={28}/>, table:<Table2 size={28}/>, stat:<Hash size={28}/>, text:<Type size={28}/> };
const INIT: Widget[] = [
...
export default function DashboardBuilderPage() {
  ...
  const add = (type:WType) => { const w:Widget={id:crypto.randomUUID(),type,title:PALETTE.find(p=>p.type===type)?.label??"Widget",w:2,h:2}; setWidgets(p=>[...p,w]); setSel(w.id); };
      ...
      <div className="gc-db">
        <div className="gc-db-palette">
          ...
          {PALETTE.map(({type,label,icon})=>(
            <div key={type} className="gc-fb-type" style={{cursor:"pointer"}} onClick={()=>add(type)}>
        ...
        <div className="gc-db-canvas">
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,gridAutoRows:"140px"}}>
            {widgets.map(w=>(
              <div key={w.id} onClick={()=>setSel(w.id)} className={`gc-widget ${sel===w.id?"selected":""}`}
                ...
                <div className="gc-widget-body" style={{height:`calc(100% - 34px)`,flexDirection:"column",gap:4}}>
                  {w.type==="stat"?<><div style={{fontSize:28,fontWeight:700,color:"var(--c-blue)"}}>{Math.floor(Math.random()*200+10)}</div><div style={{fontSize:11,color:"var(--c-text-4)"}}>{w.title}</div></>:<>{ICONS[w.type]}<div style={{fontSize:11,color:"var(--c-text-4)"}}>{w.title}</div></>}
                </div>
              </div>
            ))}
            <div onClick={()=>add("stat")} style={{border:"2px dashed var(--c-border)",borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",color:"var(--c-text-4)",cursor:"pointer",minHeight:140,transition:"border-color .15s,color .15s"}} onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.borderColor="var(--c-blue)";(e.currentTarget as HTMLElement).style.color="var(--c-blue)";}} onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.borderColor="var(--c-border)";(e.currentTarget as HTMLElement).style.color="var(--c-text-4)";}}>
              <Plus size={20}/>
            </div>
          </div>
        </div>
        <div className="gc-db-props">
          <div style={{padding:"10px 12px",borderBottom:"1px solid var(--c-border-light)",display:"flex",alignItems:"center",gap:6}}>
            <Settings size={13} style={{color:"var(--c-text-4)"}}/><span style={{fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:".06em",color:"var(--c-text-4)"}}>Widget Settings</span>
          </div>
          {selW?(
            <div style={{padding:12}}>
              <div className="gc-prop-row"><label style={{fontSize:11,color:"var(--c-text-2)",display:"block",marginBottom:3}}>Title</label><input className="gc-prop-input" value={selW.title} onChange={e=>upd(selW.id,{title:e.target.value})}/></div>
              <div className="gc-prop-row"><label style={{fontSize:11,color:"var(--c-text-2)",display:"block",marginBottom:3}}>Data Source</label><select className="gc-prop-input" value={selW.source??""} onChange={e=>upd(selW.id,{source:e.target.value})}><option value="">None</option>{SOURCES.map(s=><option key={s} value={s}>{s}</option>)}</select></div>
              <div className="gc-prop-row"><label style={{fontSize:11,color:"var(--c-text-2)",display:"block",marginBottom:3}}>Width (cols)</label><input type="range" min={1} max={4} value={selW.w} onChange={e=>upd(selW.id,{w:+e.target.value})} style={{width:"100%",accentColor:"var(--c-blue)"}}/><div style={{fontSize:11,textAlign:"right",color:"var(--c-text-4)"}}>{selW.w}/4</div></div>
              <div className="gc-prop-row"><label style={{fontSize:11,color:"var(--c-text-2)",display:"block",marginBottom:3}}>Height (rows)</label><input type="range" min={1} max={4} value={selW.h} onChange={e=>upd(selW.id,{h:+e.target.value})} style={{width:"100%",accentColor:"var(--c-blue)"}}/><div style={{fontSize:11,textAlign:"right",color:"var(--c-text-4)"}}>{selW.h} rows</div></div>
            </div>
          ):(
