...
interface Project { id: string; name: string; description?: string; status: string; form_count?: number; }
interface FormSchema { id: string; name: string; geometry_type: string; is_published: boolean; schema: { fields: unknown[] }; }
...
export default function ProjectsPage() {
  ...
  const sync = async () => {
    ...
    setSyncResult(`Synced ${r.synced} features${r.failed > 0 ? `, ${r.failed} failed` : ""}`);
    setSyncing(false);
    ...
    <div style={{ maxWidth: 480, margin: "0 auto", padding: "16px 16px 80px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, paddingTop: 8 }}>
        <div>
          ...
          {user && <p style={{ fontSize: 12, color: "#64748b" }}>{(user as {full_name:string}).full_name}</p>}
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <button className="btn btn-ghost" style={{ fontSize: 12, padding: "6px 12px" }} onClick={sync} disabled={syncing}>
            {syncing ? "Syncing…" : "⟳ Sync"}
          </button>
          <button className="btn btn-ghost" style={{ fontSize: 12, padding: "6px 12px" }} onClick={logout}>Sign out</button>
        </div>
      </div>

      {syncResult && (
        <div style={{ background: "#dcfce7", color: "#16a34a", padding: "8px 12px", borderRadius: 8, fontSize: 13, marginBottom: 12 }}>
          {syncResult}
        </div>
      )}

      {!navigator.onLine && (
        <div style={{ background: "#ffedd5", color: "#ea580c", padding: "8px 12px", borderRadius: 8, fontSize: 13, marginBottom: 12 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {projects.map(p => (
          <div key={p.id} className="card">
            <div onClick={() => expandProject(p.id)}
              ...
              <div>
                ...
                <p style={{ fontSize: 12, color: "#64748b", marginTop: 2 }}>{p.description || "No description"}</p>
              </div>
              <span style={{ fontSize: 18, color: "#94a3b8" }}>{expanded === p.id ? "▲" : "▼"}</span>
            </div>
            {expanded === p.id && (
              <div style={{ borderTop: "1px solid #e2e8f0", padding: "10px 16px 14px" }}>
                {(forms[p.id] || []).length === 0 ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {(forms[p.id] || []).map(f => (
                      <button key={f.id} className="btn btn-primary" style={{ justifyContent: "flex-start", width: "100%" }}
                        onClick={() => nav(`/collect/${p.id}/${f.id}`)}>
                        📋 {f.name} <span style={{ marginLeft: "auto", fontSize: 11, opacity: 0.8 }}>{f.geometry_type}</span>
                      </button>
                    ))}
                  </div>
                )}
